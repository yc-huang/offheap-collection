...
public class NestedOpenHashMap<K, V> implements Map<K, V>{
	...
	Map<K, V> collisionMap;

	public NestedOpenHashMap(Class<K> keyCls, Class<V> valueCls, int size) {
...
//		}
		...
		System.out.printf(this.getClass() + ":idx size:%,d, slot size:%d\n", idxSize, slotSize);
//		maxProbe = idxSize >> 12;
		maxProbe = 128;
		...
		collisionMap = new OpenHashMap<K, V>(keyCls, valueCls, idxSize >> 3);
	}
	...
	public boolean contains(K key) {
		...
		Address addr = checkExist(key);
		if(addr.slotAddr == -1) return collisionMap.contains(key);
		else return addr.recordAddr != -1;
	}
	...
	public V get(K key) {
		...
		long recordAddr = addr.recordAddr;
